...
export function Shop({
  ...
  const { status, value } = useProducts(page, limit);
  ...
  const filtered = useMemo(() => {
    ...
    let list = products.filter((p: any) => {
      if (pet !== "All" && p.petType !== pet && p.petType !== "All")
        return false;
      if (cat !== "All" && p.category !== cat) return false;
      if (q && !(p.name + p.brand).toLowerCase().includes(q.toLowerCase()))
        return false;
      return true;
    });
    ...
    return list;
  }, [q, pet, cat, sort, products]);
  useEffect(() => {
    setPage(1);
  }, [q, pet, cat]);

  const totalPages = Math.ceil((total as number) / limit) || 1;
  console.log(products);
  console.log(total);
      ...
      pages.push(
        <button
          ...
          className={`btn rounded-circle d-flex align-items-center justify-content-center p-0 ${
            page === i ? "text-white" : "btn-light bg-white text-dark border"
          }`}
          style={{
            width: "40px",
            height: "40px",
            ...
            backgroundColor: page === i ? "#0F766E" : "transparent",
            borderColor: page === i ? "#0F766E" : "#E7E2D9",
            transition: "all 0.2s",
          }}
        >
          {i}
  ...
  return (
    <div
      className="container-fluid mx-auto px-4 py-5 animate-slide-up"
    ...
    >
      ...
      {status === "error" ? (
        ...
        >
          ...
          <div className="fw-bold fs-4 mt-2 mb-2 text-dark">
        ...
        <>
          <div className="d-flex flex-wrap align-items-end justify-content-between gap-4 mb-5">
            <div>
              <h1
                className="fw-semibold text-dark lh-sm m-0"
                style={{ fontFamily: "Sora", fontSize: "32px" }}
              >
                Shop everything for your pet
              </h1>
              <p
                className="mt-2 text-muted m-0"
          ...
          <div className="row g-5">
            <aside className="col-12 col-lg-3">
              <div
                className="bg-white border p-4 sticky-top"
                style={{
                  borderRadius: "1rem",
                  borderColor: "#E7E2D9",
                  top: "88px",
              ...
              >
                <div className="d-flex align-items-center gap-2 mb-4">
                  ...
                  <div
                    ...
                    style={{ fontFamily: "Sora", fontSize: "14px" }}
                  >
                    Filters
                  </div>
                </div>
            ...
            <div className="col-12 col-lg-9">
              <div className="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
                <div className="d-flex flex-wrap gap-2">
                  {pet !== "All" && (
                    <ActiveTag onClear={() => setPet("All")}>
                      Pet: {pet}
                    </ActiveTag>
                  )}
                  {cat !== "All" && (
                    <ActiveTag onClear={() => setCat("All")}>
                      Category: {cat}
                    </ActiveTag>
                  )}
                </div>
              ...
              {filtered.length === 0 ? (
                <div
                  className="bg-white border text-center py-5 px-3"
                  style={{
                    borderRadius: "1rem",
                    borderStyle: "dashed",
                    borderWidth: "2px",
                ...
                >
                  ...
                  <div
                    ...
                    style={{ fontFamily: "Sora", fontSize: "18px" }}
                  >
                    No results for "{q}"
                  </div>
                  <p
                ...
                <>
                  ...
                  <div
                    className="d-flex justify-content-center align-items-center gap-2 mt-5 pt-4 border-top"
                    style={{ borderColor: "#E7E2D9" }}
                  >
                    <button
                      disabled={page === 1}
                      onClick={() => setPage((prev) => Math.max(prev - 1, 1))}
                      className="btn btn-light bg-white border rounded-circle d-flex align-items-center justify-content-center p-0 text-dark transition-all btn-hover-scale"
                      style={{
                        width: "40px",
                        height: "40px",
                        borderColor: "#E7E2D9",
...
function FilterGroup({ label, children, index = 1 }: any) {
  return (
    <div
      className={index === 0 ? "pb-2" : "py-2 border-top"}
      style={{ borderColor: "#E7E2D9" }}
    >
      <div
        className="fw-semibold text-uppercase text-muted mb-2"
        style={{
          fontFamily: "Manrope",
          fontSize: "12px",
...
function ActiveTag({ children, onClear }: any) {
  return (
    ...
    >
      ...
      <button
        onClick={onClear}
        className="btn rounded-circle d-flex align-items-center justify-content-center p-0 shadow-none border-0 text-white"
        style={{ width: "20px", height: "20px", backgroundColor: "#0F766E" }}
      >
        <X size={11} />
      </button>
    </span>
