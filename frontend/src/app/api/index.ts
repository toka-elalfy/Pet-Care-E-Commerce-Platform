  ...
  if (!res.ok) {
    ...
    try {
      ...
      if (Array.isArray(errData) && errData.length > 0) {
        ...
        errStr =
          errData.message ||
          (typeof errData.err === "string" ? errData.err : errData.msg) ||
          errStr;
      }
    } catch {}
...
export const api = {
  user: {
    updateProfile: async (data: any) =>
      fetchWithAuth("/user/profile", { method: "PUT", data }),
    updatePassword: async (data: any) =>
      fetchWithAuth("/user/password", { method: "PUT", data }),
    deleteAccount: async () =>
      fetchWithAuth("/user/account", { method: "DELETE" }),
    addAddress: async (data: any) =>
      fetchWithAuth("/user/addresses", { method: "POST", data }),
    updatePrimaryAddress: async (id: string) =>
      fetchWithAuth(`/user/addresses/${id}/primary`, { method: "PUT" }),
    removeAddress: async (id: string) =>
      fetchWithAuth(`/user/addresses/${id}`, { method: "DELETE" }),
  },
  auth: {
    login: async (data: any) =>
      fetchWithAuth("/auth/login", { method: "POST", data }),
    register: async (data: any) =>
      fetchWithAuth("/auth/register", { method: "POST", data }),
  ...
  categories: {
    getAll: async (): Promise<string[]> => {
      ...
      try {
        ...
        console.error(
          "Failed to fetch categories, falling back to basic:",
          err,
        );
        return ["All", "Food", "Toys", "Health", "Grooming"];
      }
  ...
  cart: {
    ...
    addToCart: async (
      ...
      fetchWithAuth("/cart/add", {
        method: "POST",
        data: { productId, quantity, purchaseType, frequency },
      }),
  ...
  products: {
    getAll: async (
      ...
      try {
        ...
        if (data.length === 0) {
          console.error(
            "Warning: Backend returned 0 products. Falling back to default list.",
          );
          return { products: products, total: products.length };
        }
  ...
  pets: {
    ...
    create: async (data: any) =>
      fetchWithAuth("/pets", { method: "POST", data }),
    update: async (id: string, data: any) =>
      fetchWithAuth(`/pets/${id}`, { method: "PATCH", data }),
    delete: async (id: string) =>
      fetchWithAuth(`/pets/${id}`, { method: "DELETE" }),
    getRecommendations: async (id: string) =>
      fetchWithAuth(`/pets/recommendations/${id}`),
  },
  ...
  subscriptions: {
    ...
    updateStatus: async (id: string, data: any) =>
      fetchWithAuth(`/subscriptions/${id}/status`, { method: "PUT", data }),
    skipDelivery: async (id: string) =>
      fetchWithAuth(`/subscriptions/${id}/skip`, { method: "PUT" }),
    update: async (id: string, data: any) =>
      fetchWithAuth(`/subscriptions/${id}`, { method: "PUT", data }),
  },
  reminders: {
    getAll: async () => {
      if (USE_MOCK) {
      ...
      try {
        ...
        const data =
          res.reminders || res.data || (Array.isArray(res) ? res : []);
        ...
        if (Array.isArray(data.upcomingDeliveries)) {
          data.upcomingDeliveries.forEach((sub: any) => {
            list.push({
              ...
              left: sub.nextDeliveryDate
                ? `Ships ${new Date(sub.nextDeliveryDate).toLocaleDateString()}`
                : "Arriving soon",
            });
          });
        }
